...
export function MonthSwitcher({ month }: { month: string }) {
  ...
  return (
    <div className="flex w-full min-w-0 items-center rounded-xl border border-slate-200 bg-white shadow-sm sm:w-56">
      <button
        className="grid size-10 shrink-0 place-items-center rounded-l-xl text-slate-500 transition hover:bg-slate-50 hover:text-slate-900"
        onClick={() => shift(-1)}
      ...
      <button
        className="grid size-10 shrink-0 place-items-center rounded-r-xl text-slate-500 transition hover:bg-slate-50 hover:text-slate-900"
        onClick={() => shift(1)}
